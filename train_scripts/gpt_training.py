...
# Shuffle the training set and select a few training samples for mlflow logging 
...
def group_text(examples, contex_length=512):
    # Concatenate all texts.
    concatenated_text = {k: sum(examples[k], []) for k in examples.keys()}
    total_length = len(concatenated_text[list(examples.keys())[0]])
    total_length = (total_length // contex_length) * contex_length

    result = {
        k: [t[i : i + contex_length] for i in range(0, total_length, contex_length)]
...
# Group the tokenized training dataset
